...
int i2c_read_seq(i2c_seq_t *seq, int seqn, uint8_t *outdata, int maxlen)
{
#if defined( CORTEX )
	int i = 0;
	int offset = 0;
	...
	while (seq[i].bOperation == 0x01)
	{
		if ((offset + seq[i].bLen) > maxlen) return I2C_OVF_BUFFER;
		ret = i2c_read(seq[i].bDevAddr, seq[i].bAddr, &outdata[offset], seq[i].bLen);
		if (ret) return ret;
		offset += seq[i].bLen;
		i++;
	}
	...
	data_io->pc2dev.bOperation = OP_I2C_SEQ_READ;
	data_io->pc2dev.bDevAddr = 0;
	data_io->pc2dev.bAddr = 0;
	data_io->pc2dev.bLen = seqn;
	memcpy(data_io->pc2dev.abData, (uint8_t*)seq, seqn * sizeof(i2c_seq_t));
...
rep_read_answer_r:
	...
	memcpy(outdata, data_io->dev2pc.abData, data_io->dev2pc.bLen);
	return data_io->dev2pc.bRetStatus;
#endif
}
